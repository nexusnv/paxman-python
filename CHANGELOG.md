...
- **Money — `dollar_sign_currency` constrained to own candidates (#15):** `Money` mirrors `Currency` per ratified ruling — a shared bare symbol resolves to the opt-in code only when it is one of the token's own CLDR candidates (`$500`+`USD` → `SUCCESS`; `$500`+`MYR` → `INVALID`, was `SUCCESS`; single-candidate tokens and default-`None` → `INVALID` unchanged). Guard in both resolvers, `money.md` parity rewrite, README opt-in example.
