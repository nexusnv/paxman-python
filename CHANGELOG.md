...
- **MacAddress — de-offer `bit_reversed` output format (#123, ADR-0010):** `MacAddressContract(output_format="bit_reversed")` now raises `ContractError` (was `SUCCESS` with per-octet bit-swap). The format is an involution (`f(f(x)) == x`) and therefore not a fixed point — `canonicalize(V, bit_reversed)` re-flips to `W != V`. Per ADR-0010 an offered format must re-enter as itself, so the view is no longer offered. The `Capability.format_value()` `bit_reversed` branch remains for direct use but is no longer reachable via the pipeline contract. Migrate: use the default `colon` form for storage and round-tripping; compute the Token-Ring display via `_bit_reverse_octet` locally if needed.
