...
def test_view_identity_no_offsets() -> None:
    ...
    view = ctx.view("casefolded", lambda t: (t.lower(), None, None))
...
def test_view_offset_map_invariant_length_changing() -> None:
    ...
    assert view.source_starts == (0, 3)
    assert view.source_ends == (1, 4)
    assert view.source_starts is not None
    assert len(view.source_starts) == len(view.subject)
    assert view.original_span(0, 1) == (0, 1)
...
def test_word_spans_shared_across_views() -> None:
    ...
    v1 = ctx.view("v1", lambda t: (t.lower(), None, None))
    v2 = ctx.view("v2", lambda t: (t.upper(), None, None))
