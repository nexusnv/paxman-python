...
- Registry hygiene: integration + e2e suites use an autouse `_clean_registry` fixture calling `reset_registry()`; `test_discovery.py` resets it per test. Property tests never touch the registry (they drive grammars/rules/`format_value` directly) — the three documented exceptions are `test_money_properties.py`, `tests/property/test_reentry_invariant.py`, and `tests/property/test_coordinates_quantization.py` (declared-quantization fixpoint + bounded-drift pre-image — also a full-pipeline invariant), which lock full-pipeline invariants with a local `_fresh_registry` fixture (documented in their module docstrings; `test_reentry_invariant.py` mirrors the `test_money_properties.py` `_fresh_registry` pattern).
