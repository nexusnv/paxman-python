...
3. Define a class that extends `Capability`
 ...
 8. Implement `format_value(value, output_format, notation)` — the single presentation seam. The engine calls it once per surviving candidate at result assembly — after candidate deduplication and status determination, which read the canonical, pre-format values — so `output_format` never affects candidate identity. Return the value unchanged for the default format; only an explicitly offered alternative triggers conversion (e.g., Date `"US"` rendering, Country `alpha3`/`numeric`/`name` conversion, Phone RFC 3966/national rendering). A capability with no alternative formats (e.g., Email, IP) inherits the identity implementation from `Capability` and does not override it. Rules never implement presentation — see Step 5d and the presentational-only invariant in Step 7.
...
`output_format` is a *representation* transform, never a *recognition* or *validation* signal. Rules own validation and default normalization only: `matches()` never consults `output_format`, and `normalize()` always returns the default canonical form. Presentation lives in one place — the capability's `format_value(value, output_format, notation)` method — which the engine calls once per surviving candidate at result assembly, after candidate deduplication and status determination (which read the canonical, pre-format values). The format choice cannot change which candidates exist or which canonical values they carry; it only changes how each candidate is rendered. This is the contract mandate: the pipeline reports what authoritative specifications say, regardless of how the caller wants the answer displayed. Concretely:

- **Rules validate and normalize; the capability formats; the engine counts.** Each rule's `normalize()` returns the default canonical value; the engine dedups and computes status from the set of distinct canonical values; `Capability.format_value()` then renders each surviving candidate in the requested format for the surfaced result. No grammar is re-run, no input is re-parsed, and no rule is re-invoked.
...
#### ADR-0011 class-declaration checklist (offered formats)

Every offered format is an **encoding**, a **same-entity expansion**, or a **documented quantization** — never a projection over recognition/validation information (ADR-0011, Information-Preservation Invariant). Before offering a format:

1. **Declare its class at the contract seam.** Name the format in the contract class docstring and state its class (`encoding` / `same-entity expansion` / `documented quantization`; projections are not offered — a deliberately waived projection needs an ADR-recorded waiver instead). `tests/unit/test_offered_format_class_declarations.py` makes this structural: every offered format must share a docstring paragraph with its class term (a class word elsewhere in the docstring does not cover it), and only a recorded waiver holder may mention projections.
2. **Argue the param-free pre-image in the capability's tests.** Encodings: the rendering re-enters the *default* (param-free) contract exactly. Expansions: merge-and-fixpoint — same-entity spellings render identically and each rendering re-enters to itself. Quantizations: declare the render quantum at the contract docstring and lock render-stability + bounded-drift (≤ ½ render quantum + ½ canonical quantum) property tests.
3. **Add the row to the preservation matrix.** `tests/property/test_output_format_preservation.py`'s `CLASS_MAP` needs a `(capability, format)` entry (an unclassified offered format fails the module gate loudly), plus a cross-entity injectivity pair in `_INJECTIVITY_PAIRS` (documented quantizations are excluded from injectivity by class).
