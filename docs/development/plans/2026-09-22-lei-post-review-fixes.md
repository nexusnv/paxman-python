...
- [ ] `lei.md`: add recognition-table row for URL-path-embedded input https://search.gleif.org/#/record/5493000IBP32UQZ0KL24 under Recognizes; add statuses row with that URL | defaults | SUCCESS | carved at the / and # boundary — URL-level semantics belong to the URL capability; in the deferred section link the two GitHub Issues from Task 5 (hyphen-grouped, issued/live). Re-execute the guide snippet + new row values and paste measured output (no assumed values).
