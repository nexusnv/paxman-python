...
1. **End-lists, sorted ascending, branching only in `seq`.** `leaf_maps` becomes `dict[int, dict[int, list[int]]]` (deduped, sorted). `seq` tries each end of a leaf child recursively (depth-first, first-success wins to preserve current preference order); `alt`/`opt`/`rep`/`label` keep calling a first-hit wrapper so their semantics don't change. Top-level `match()` keeps first-success per start (no new spans on shipped grammars — goldens prove it).
