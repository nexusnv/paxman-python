...
1. **Disqualification, not ranking.** No rule is superior to another; no scores/weights/priority fields anywhere. A PARSER candidate is *provisional structure*, kept iff a LOOKUP_TABLE rule validates the same recognition. Rationale: unregistered tags denote nothing — the multi-candidate extension of the existing recognized-but-unvalidated → `INVALID` rule (`_determine_status`: no candidates + recognitions → `INVALID`). **Vacuity (amended post-Task-3-block):** the filter applies to a recognition only when ≥1 *active* LOOKUP rule — from the post-`_filter_rules` `rules` list in `run_capability` (`:124`), so `pinned_rules`/`excluded_rules`/`year`/`requires_features` gating is honored — targets its semantics. Where no authority is in force (all-PARSER capabilities; filtered-out lookup), PARSER stands as today. Corroborating precedent: `en-x-private` without `include_private` flips `SUCCESS` → `INVALID` (two-locus model already resolves gated-off authority to `INVALID`: `qaa`/`aav`/`allemand`); with the flag it stays `SUCCESS`.
...
**Goal:** Drop PARSER candidates no *active* LOOKUP rule corroborates on the same recognition; pure function of (candidate, rep, rule strategies), no ordering, no scores. Zero status changes outside Language.

- [ ] Implement: group collected pairs by recognition identity; for each recognition, check the already-filtered `rules` list in `run_capability` scope (NOT `all_rules` — year/feature gating must be honored): if no active LOOKUP_TABLE rule targets its semantics, keep all its pairs untouched (vacuity); else drop PARSER-strategy pairs with no same-recognition LOOKUP_TABLE pair; LOOKUP_TABLE/REGEX/unknown-rule pairs always kept. Uncommitted prior attempt in the tree may be adapted (it lacks only the vacuity guard and used the wrong rule list — verify, don't assume). Also flip `test_extlang_and_private`'s no-flag `en-x-private` expectation to `INVALID` with a two-locus comment (gated-off authority; with-flag stays `SUCCESS`); `test_year_filter` year=2009 stays `SUCCESS` via vacuity. Verify: Task 2 tests → PASS; `uv run pytest tests/integration -q` → PASS with zero non-Language status changes (any = BLOCKED); `uv run pytest tests/property/test_reentry_invariant.py -q` → PASS; `uv run ruff check paxman/engine/ tests/integration/test_language_capability.py && uv run ruff format --check paxman/engine/orchestrator.py tests/integration/test_language_capability.py`; `uv run pyright paxman/engine/orchestrator.py` → 0 errors. Probes (`register_all_shipped`): `xx-yyyyy` → INVALID, `Serbo-Croatian` → SUCCESS sh, `en-US` → SUCCESS both rules, `en-US` year=2009 → SUCCESS, `en-x-private` default → INVALID / with flag → SUCCESS; commit: `fix(engine): disqualify uncorroborated PARSER candidates (ADR-0012)`.
