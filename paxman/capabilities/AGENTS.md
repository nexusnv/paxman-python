...
| Generated data | only modules with a source snapshot + tool (ISBN range message → `tools/regenerate_isbn_range_data.py`, SIUnit prefixed units → `tools/regenerate_si_prefix_data.py`, URL IDNA UTS #46 mapping → `tools/regenerate_idna_uts46_data.py`, Currency + Money data set → `tools/regenerate_currency_data.py` from `paxman/shared_data/currency_snapshot.json`, BIC data → `tools/regenerate_bic_data.py`, IBAN registry → `tools/regenerate_iban_registry_data.py`, Language tables → `tools/regenerate_language_data.py` (`tools/regenerate_iana_language_data.py` now guards removed generated modules), Unicode property ranges → `tools/regenerate_unicode_property_data.py`) — edit via the snapshot, then regenerate |
...
- **Data files are plain tables, most maintained in place** — `rules/data/` and `grammar/data/` exist to separate data from logic, not to mark generated output. Only modules that carry a generator (source snapshot + script — currently the ISBN range message via `tools/regenerate_isbn_range_data.py`, the SIUnit prefixed-unit and grammar token tables via `tools/regenerate_si_prefix_data.py`, the URL IDNA UTS #46 mapping via `tools/regenerate_idna_uts46_data.py`, the Currency + Money data set via `tools/regenerate_currency_data.py` from `paxman/shared_data/currency_snapshot.json`, BIC data via `tools/regenerate_bic_data.py`, the IBAN registry via `tools/regenerate_iban_registry_data.py`, Language tables via `tools/regenerate_language_data.py` with `tools/regenerate_iana_language_data.py` guarding removed modules, and Unicode property ranges via `tools/regenerate_unicode_property_data.py`) must be edited through the snapshot and regenerated, never by hand, or they drift from their authority. Unmarked data files are edited directly.
...
## NOTES
- `__init__.py` exports all twenty-four shipped capabilities; completeness is enforced by `tests/unit/test_capability_exports.py`. IBAN, ISSN, ISIN, DOI, and LEI are the minimal-surface members (1 grammar + 1–2 rules each).
