...
├── capabilities/   # 25 self-contained capability packages
...
| `register_all_shipped()` / `list_shipped_capabilities()` | functions | `paxman/api/bootstrap.py` | One-call registration of the 25 shipped capabilities; deterministic name list |
...
## NOTES
- `paxman/capabilities/__init__.py` exports all twenty-five shipped capabilities (BIC, ChemicalElement, Coordinates, Country, Currency, Date, DOI, Email, GTIN, IBAN, IP, ISBN, ISIN, ISSN, Language, LEI, MacAddress, Money, ORCID, Phone, SI Unit, Timezone, URL, UtcOffset, UUID); export completeness is enforced by `tests/unit/test_capability_exports.py`.
- CONTEXT.md is the domain glossary for the full shipped set (twenty-five capabilities). It is kept in sync with the code; when adding a capability, update its Notation/table entries there too.
