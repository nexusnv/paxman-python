...
├── capabilities/   # 21 self-contained capability packages
...
| `register_all_shipped()` / `list_shipped_capabilities()` | functions | `paxman/api/bootstrap.py` | One-call registration of the 21 shipped capabilities; deterministic name list |
...
## NOTES
- `paxman/capabilities/__init__.py` exports all twenty-one shipped capabilities (BIC, Coordinates, Country, Currency, Date, Element, Email, IBAN, IP, ISBN, ISSN, Language, MacAddress, Money, ORCID, Phone, SI Unit, Timezone, URL, UtcOffset, UUID); export completeness is enforced by `tests/unit/test_capability_exports.py`.
- CONTEXT.md is the domain glossary for the full shipped set (twenty-one capabilities). It is kept in sync with the code; when adding a capability, update its Notation/table entries there too.
